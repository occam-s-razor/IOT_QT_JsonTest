...
public:
    ...
    {//构建json对象
        ...
        lightObject.insert("light1", getLight1()?"on":"off");
        lightObject.insert("light2", getLight2()?"on":"off");
        ...
        vluesObject.insert("light",QJsonValue(lightObject));

        // 构建 Json 对象 - json
        jsonobject.insert("type",type);
        jsonobject.insert("gateway_id",gateway_id);
        jsonobject.insert("vlues",QJsonValue(vluesObject));
